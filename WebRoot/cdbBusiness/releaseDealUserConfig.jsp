	...
	<script language="javascript">
		...
		*/
		...
		function del()
		{
			...
			if(confirm("确定删除?"))
			{
				jQuery.post('${ctx}/business/delReleaseDealUserConfig.action',
				{
					ids:temp
				},
				function(data){
		...
		function toAdd(ids){
			...
			openwindow('${ctx}/business/addReleaseDealUserConfig.action?ids=' + ids,'',800,400);
		}
	</script>
  </head>
  <body>
  	<dg:datagrid var="userinfo" title="发布工单处理人配置" action="" sqlname="SQL_CDB_ReleaseDealUserConfig.query" cachemode="sql">
  		<dg:lefttoolbar>
		...
		<dg:gridrow>
			<tr class="${userinfo_row}">
		        ...
		        <td onclick="toAdd('${pid}');">${confirmname}</td>
		        <td onclick="toAdd('${pid}');">${auditusers}</td>
		        <td>${organizeauditname}</td>
		        <td>${auditusers_2}</td>
		        <td>${excuteuser}</td>
		        <td>${testuser}</td>
		    </tr>
