...
<body>
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table hasthbg">
        <tbody>
          <tr>
            ...
            <th>工单主题</th>
            <th style="width:12%">处理组/人 </th>
            <th style="width:22%">派发时间 </th>
	...
	<div style="overflow-x:hidden;overflow-y:auto;width:100%;height:392px;">
		<table width="100%" cellspacing="0" cellpadding="0" border="0" class="list_table hasthbg" id="tab">
			<tbody>
				...
				<%
					...
					String createTime = TimeUtils.formatIntToDateString(row.getLong("CREATETIME"));
					String stdate = TimeUtils.formatIntToDateString(row.getLong("stdate"));
					String assign = (String) row.getRowHashMap().get("ASSGINEE");
					String creatorname = (String) row.getRowHashMap().get(
							"BASECREATORNAME");
					String groupname = (String) row.getRowHashMap()
							.get("GROUPNAME");
					String baseid = (String) row.getRowHashMap().get("BASEID");
					String taskid = (String) row.getRowHashMap().get("TASKID");
					String processtype = (String) row.getRowHashMap().get(
							"PROCESSTYPE");
					String baseschema = (String) row.getRowHashMap().get(
							"BASESCHEMA");
					Long duedate = row.getLong("BASEDEALOUTTIME");
					Long currenttime = System.currentTimeMillis()/1000;
				%>
				<tr style="cursor: pointer;"
					onclick="openSheet('<%=baseschema%>','<%=baseid%>','<%=taskid%>','<%=processtype%>');">
					...
					<%
						if (!"".equals(assign)) {
					%>
						<td style="width:12%"><%=assign%></td>
					<%
						}else if (!"".equals(groupname)) {
					%>
						<td style="width:12%"><%=groupname%></td>
					<%
						}else{
					%>
					...
					%>
					<td style="width:22%"><%=stdate%></td>
					<td style="width:10%"><%=basestatus%></td>
					<td style="width:20%"><%=basesn%></td>
					<%-- <td>
						<% if(duedate > 0 && duedate < currenttime){%>
							<img src="${ctx}/workflow/sheet/images/overtime.gif"
							title="<eoms:lable name="wf_sheet_unfinished_timeout" />" />
						<%} %>
					</td> --%>
				</tr>
				<%
		...
		%>
			...
			<select class='paddingHack' id='var_currentpage'
				name='var_currentpage' onchange="return tranferPageWait('go')">
				<%
			...
			<select class='paddingHack' id='var_pagesize' name='var_pagesize'
				onchange="return tranferPageWait('setsize')">
			...
			条&nbsp;
               ...
               <div class="page">
                <span>
                <a href="javascript:;" onclick="return tranferPageWait('frist')" target="_self">第一页&nbsp;</a>
			<a href="javascript:;" onclick="return tranferPageWait('previous')"
				target="_self"> &nbsp; 上一页 </a>
			&nbsp;&nbsp;
			<a href="javascript:;" onclick="return tranferPageWait('next')" target="_self">下一页 </a>
			<a href="javascript:;" onclick="return tranferPageWait('last')"
				target="_self"> &nbsp;最后页&nbsp;</a>
                </span>
                </div>
              </div>
	...
	<form id="sheetform" action="${ctx}/sheet/openWaittingSheet.action"
