...
<html>
	...
	<body>
		<div class="content">
			...
			<%if("2".equals(sortType)||"5".equals(sortType)||"6".equals(sortType)){ %>
			tree.setXMLAutoLoading("${ctx}/ultrarepository/repositoryTree.action");//  
			tree.loadXML("${ctx}/ultrarepository/repositoryTree.action?id=0");//repositoryTree=网络类型，repositoryComplaintTypeTree==投诉分类
            <%}else if("7".equals(sortType)){%>
            tree.setXMLAutoLoading("${ctx}/ultrarepository/repositoryComplaintTypeTree.action");//  
			tree.loadXML("${ctx}/ultrarepository/repositoryComplaintTypeTree.action?id=0");//repositoryTree=网络类型，repositoryComplaintTypeTree==投诉分类
            <%}%>
				...
				if(ids!=''){
					...
					for (var i = 0; i < idArr.length; i++) {
					    if(idArr[i].indexOf("_") > 0 )   
      					...
      					if (isAllPathPara == '1') {
      						var text = tree.getUserData(idArr[i],"allText");
      					} else {
