...
function dhtmlXWindows() {
	...
	this.createWindow = function (id, x, y, width, height) {
		...
		win.removeUserButton = function (id) {
			if (!((id == "minmax1") || (id == "minmax2") || (id == "park") || (id == "close") || (id == "stick") || (id == "unstick") || (id == "help"))) {
				...
				if (btn != null) {
					that._removeUserButton(this, id, btn);
				}
			}
		};
		...
		win.attachTabbar = function () {
			...
			this.tabbar.cells = function (id, name) {
				return this._cells.call(this, that, id, name);
			};
			return this.tabbar;
		};
		...
		win.attachLayout = function (view, skin) {
			...
			if (w == 0) {
				w = parseInt(this._content.style.width);
			}
			obj.style.left = "0px";
			obj.style.top = "0px";
	...
	this._diableOnSelectInWin = function (obj, state) {
		for (var q = 0; q < obj.childNodes.length; q++) {
			...
			if (child.childNodes.length > 0) {
				this._diableOnSelectInWin(child, state);
			}
			child = null;
		}
	};
	this._redrawWindow = function (win) {
		...
		var tdVis = 0;
		for (var q = 0; q < trObj.childNodes.length; q++) {
			if (trObj.childNodes[q].className == "dhtmlx_wins_btn_visible") {
				tdVis++;
			}
		}
		var wdth = win.w - tdVis * 18 - 30;
		if (wdth < 0) {
			wdth = 0;
		}
	...
	this._fixWindowPositionInViewport = function (win) {
		...
		if (win._keepInViewport) {
			...
			if (win.y + skin["header_height"] > this.vp.offsetHeight) {
				win.y = this.vp.offsetHeight - skin["header_height"];
			}
			if (win.y < 0) {
				win.y = 0;
			}
	...
	this._showWindowHeader = function (win) {
		...
		for (var a in win._skinParams) {
			delete win._skinParams[a];
		}
		win._skinParams = null;
		var h = this.skinParams[this.skin]["header_height"] + "px";
		win.childNodes[0].childNodes[0].childNodes[0].childNodes[0].style.height = h;
		win.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].style.height = h;
		var hdr = win.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];
		for (var q = 0; q < hdr.childNodes.length; q++) {
			hdr.childNodes[q].style.height = h;
		}
		this._redrawWindow(win);
	};
	this._hideWindowHeader = function (win) {
		...
		win._skinParams = {};
		for (var a in this.skinParams[this.skin]) {
			win._skinParams[a] = Number(this.skinParams[this.skin][a]).valueOf();
		}
	...
	this._closeWindow = function (win) {
		if (this._focusFixIE) {
			this._focusFixIE.style.top = win.y + "px";
	...
	this._attachContent = function (win, type, obj, append) {
		...
		if (type == "url") {
			...
			win._frame = fr;
			if (_isIE) {
				win._frame.onreadystatechange = function (a) {
					if (win._frame.readyState == "complete") {
	...
	this._blockSwitcher = function (state) {
		for (var a in this.wins) {
			...
			var cover = null;
			for (var q = 0; q < winContent.childNodes.length; q++) {
				if (winContent.childNodes[q].className == "dhx_content_cover_blocker") {
					cover = winContent.childNodes[q];
				}
			}
			if (cover != null) {
	...
	if (_isIE&&false) {
		...
		this._focusFixIE.style.top = "1px";
