...
keel.UploadPanel = function(cfg){
	...
	this.gp = new Ext.grid.GridPanel({
	    ...
	    columns: [
	        ...
	        {header: '类型', width: 60, sortable: true,dataIndex: 'type', menuDisabled:true},
	        {header: '大小', width: 100, sortable: true,dataIndex: 'size', menuDisabled:true,renderer:this.formatFileSize},
	        {header: '进度', width: 140, sortable: true,dataIndex: 'percent', menuDisabled:true,renderer:this.formatProgressBar,scope:this},
	...
	this.setting = {
		...
		file_size_limit : this.fileSize || (1024*1024) ,//上传文件体积上限，单位MB
	...
	keel.UploadPanel.superclass.constructor.call(this,{				
		tbar : [
			...
			{text:'开始上传',ref:'../uploadBtn',iconCls:'up',handler:this.startUpload,scope:this},'-',
			{text:'停止上传',ref:'../stopBtn',iconCls:'delete',handler:this.stopUpload,scope:this,disabled:true},'-',
			{text:'完成上传',ref:'../okBtn',iconCls:'ok',handler:this.uploadOK,scope:this},'-'
		],
		layout : 'fit',
		items : [this.gp],
		listeners : {
...
Ext.extend(keel.UploadPanel,Ext.Panel,{
	...
	uploadOK : function(){
		...
		this.swfupload.uploadStopped = false;
	    uploadWin.close();
	    if(parentStore&&parentStore!=null){
	     parentStore.reload();
	    }
	},
