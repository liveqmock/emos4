  ...
  <body>
  	<div class="content">
		<div class="page_div_bg">
			<table style="height:100%"><tr valign="bottom"><td>岗位上下级关系树</td></tr></table>
		</div>
		...
		<script type="text/javascript">
			...
			tree.setXMLAutoLoading("${ctx}/dutyOrgLevel/orgLevelTree.action");
			tree.loadXML("${ctx}/dutyOrgLevel/orgLevelTree.action?id=0");
			tree.setOnClickHandler(doHandler)
			function doHandler(key)
			{
				var name = tree.getSelectedItemText();
		    	var id = tree.getSelectedItemId();
				window.open("${ctx}/dutyOrgLevel/orgLevelManage_right.action?id="+id,"rightFrame2");
			}
			function refresh(itemid)
			{
				tree.refreshItem(itemid);
				window.open("${ctx}/dutyOrgLevel/orgLevelManage_right.action?id="+itemid,"rightFrame2");
			}
		</script>
  </body>
</html>
