...
var starttime = url.split('strattime=')[1].replace('-','/').replace('-','/');
...
function buildTask(pid,buildDate, shiftName, shiftTime, userInfoName, chargePInfoName, type,appType,userPhone) {
	...
	for (var i = 0; i < shiftName.length; i++) {
		...
		taskInfo = taskInfo + "<tr><td colspan='2' class='dutyworktime'><span>"
			+ shiftName[i] + "</span></td></tr><tr><td>"+ shiftTime[i] +"<td></tr><tr><td style='padding-left:5px;' title='"+userPhone[i]+"'><font color='"+showFontColor(appType)+"'>" ;
   ...
   if(document.getElementById(id)==null)  
     $("#" + buildDate).parent().append("<div id='" + buildDate + "'  class='task' >" + taskInfo + "<input type='hidden' id='appType_"+buildDate+"' value='"+appType+"'/><input type='hidden' id='pid_"+buildDate+"' value='"+pid+"'/></div>");
   else
 ...
 */	
 function showMonthCalendar(calflag){
 	    var ruleId = $("#ruleId").val();
		window.location.href('calendarList.action?calflag='+calflag+'&ruleId='+ruleId);
 }
