...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		function del()
		{
			...
			if(pids==''){
				...
				if(confirm("<eoms:lable name='com_btn_confirm_del'/>"))
				{
					...
					window.location.href('${ctx}/smOrderDuty/delSmOrderDuty.action?pids='+pids);
				}
			}	
		}
	</script>
	...
	<body>
		<dg:datagrid var="smOrderDuty" sqlname="SQL_SM_SmOrderDutyList.query" title="${nodePath}">
			<dg:lefttoolbar>
				...
				<eoms:operate cssclass="page_add_button" id="com_btn_add" onmouseover="this.className='page_add_button_hover'" 
  		  			...
  		  			onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action','',600,300);"
			...
			<dg:gridtitle>
				<tr>
					...
					<th width="20%"><eoms:lable name="sm_lb_dutyRoom"/></th>
					<th width="10%"><eoms:lable name="sm_lb_aheaddaynum"/></th>
					<th width="10%"><eoms:lable name="com_lb_starttime"/></th>
					<th width="10%"><eoms:lable name="com_lb_endtime"/></th>
					<th width="10%"><eoms:lable name="sm_lb_isholidayactive"/></th>
					<th width="10%"><eoms:lable name="com_lb_status"/></th>
				</tr>
			</dg:gridtitle>
			<dg:gridrow>
				<tr class="${smOrderDuty_row}">
					<td><input name="checkid" type="checkbox" value='${pid}'></input></td>
					<td><a href="javascript:;" onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action?smorderdutyPid=${pid}','',600,300);">${dutyroomname}</a></td>
					<td><a href="javascript:;" onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action?smorderdutyPid=${pid}','',600,300);">${aheaddaynum}</a></td>
					<td><a href="javascript:;" onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action?smorderdutyPid=${pid}','',600,300);">${starttime}</a></td>
					<td><a href="javascript:;" onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action?smorderdutyPid=${pid}','',600,300);">${endtime}</a></td>
					<td><a href="javascript:;" onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action?smorderdutyPid=${pid}','',600,300);"><eoms:dic dictype="isHoliday" value="${isholiday}"/></a></td>
					<td><a href="javascript:;" onclick="openwindow('${ctx}/smOrderDuty/smOrderDutySave.action?smorderdutyPid=${pid}','',600,300);"><eoms:dic dictype="status" value="${status}"/></a></td>
				</tr>
			</dg:gridrow>
		</dg:datagrid>
	</body>
