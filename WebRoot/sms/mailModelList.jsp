	...
	<head>
		...
		<script language="javascript">
			...
			function del(pid){
				...
				if(confirm("是否确认删除？")){
					window.location.href('${ctx}/mailModel/mailModelDel.action?pid='+pid);
				}
			}
		</script>
	</head>
	<body>
		<dg:datagrid var="smsmonitor" sqlname="SQL_SM_MailModelList.query" title="当前位置：邮件模板查询" action="">
			<dg:lefttoolbar>
				<eoms:operate cssclass="page_add_button" id="com_btn_add" onmouseover="this.className='page_add_button_hover'" 
		  			...
		  			onclick="openwindow('${ctx}/mailModel/mailModelAdd.action','',1000,600);"
			...
			<dg:gridrow>
				<tr class="smsmonitor_row">
					<td>${rowindex}</td>
					<td>${modeltype}</td>
					<td onclick="openwindow('${ctx}/mailModel/mailModelUpdate.action?pid=${pid}','',1000,600);">${content}</td>
					<td>${modifytime}</td>
					<td>${modifier}</td>
