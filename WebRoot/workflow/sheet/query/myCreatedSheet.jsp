...
<html>
  ...
  <body style="background-color:white;">
	  	<dg:datagrid  var="waitingdealsheet"  title="${nodePath}" action="" sqlname="${queryname}" tquery="workSheetTitle">
	  		<dg:lefttoolbar>
	  		  ...
	  		  <s:if test='%{baseSchema != null && baseSchema != ""}'>
		     	...
		     	<eoms:operate cssclass="page_saves_button" id="page_saves_button" onmouseover="this.className='page_saves_button_hover'" onmouseout="this.className='page_saves_button'"  onclick="showsearchcondition('${ctx}','${queryname}')" text="配置" />
		     	<!-- 
		      ...
		      <eoms:operate cssclass="page_help_button" id="com_btn_help" onmouseover="this.className='page_help_button_hover'" onmouseout="this.className='page_help_button'" text="com_btn_help"/>
		       -->
		       <eoms:operate cssclass="page_excel_button" id="com_btn_excel" onmouseover="this.className='page_excel_button_hover'" onmouseout="this.className='page_excel_button'" onclick="excelExport('${queryname}','${wfSortId}','${baseSchema}')" text="com_btn_export"/>
  			</dg:lefttoolbar>
	  	<dg:condition>
		  	<div align="center">
	    ...
	    <dg:gridtitle>
		    	<tr>
		    		<th width="9%" field="basesn"><eoms:lable name="wf_sheet_serialnum"/></th>
		    		<th width="" field="basesummary">主题</th>
		    		<th width="6%" field="basecreatorfullname"><eoms:lable name="wf_sheet_creator"/></th>
		    		<th width="7%" field="deal"><eoms:lable name="wf_sheet_deal"/></th>
		    		<th width="9%" field="basecreatedate"><eoms:lable name="wf_sheet_createtime"/></th>
		    		<th width="9%" field="basedealouttime"><eoms:lable name="wf_sheet_dealtime"/></th>
		    		<th width="6%" field="basestatus"><eoms:lable name="wf_sheet_currentstatus"/></th>
		    		<th width="7%">跟踪</th>
		    	</tr>
	    </dg:gridtitle> 
		<dg:gridrow>
			<tr class="${waitingdealsheet_row}">
		        <td style="text-align:center;"><a href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');">${basesn}</a></td>
		        <td><a title="${basesummary}" href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');">${basesummary}</a></td>
		        <td style="text-align:center;"><a href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');">${basecreatorfullname}</a></td>
		        <td style="text-align:center;"><a href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');">${deal}</a></td>
		        <td style="text-align:center;"><a href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');"><eoms:date value="${basecreatedate}"/></a></td>
		        <td style="text-align:center;"><a href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');"><eoms:date value="${basedealouttime}"/></a></td>
		        <td style="text-align:center;"><a href="javascript:;" onclick="openSheet('${baseschema}','${baseid}');">${basestatus}</a></td>
