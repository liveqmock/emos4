...
<%@ page import="com.ultrapower.eoms.ultrabpp.model.BaseField"%>
...
<%
	...
	if(StringUtils.isBlank(baseid)) {return;}
	String fid = StringUtils.isBlank(request.getParameter("fid")) ? "" : request.getParameter("fid");
 %>
<html>
	<head>
...
<SCRIPT language=javascript>
	...
	function controlbodyHeight()
	{
		try
		{
			...
			//alert(document.getElementById('info_content').firstChild.clientHeight);// + "=" + document.body.firstChild.lastChild.offsetHeight);
			var iframeHeight = (parseInt(parseInt(document.body.firstChild.lastChild.offsetTop) + parseInt(document.body.firstChild.lastChild.offsetHeight)) + 20);
			parent.$("#<%=fid%>").css("height", iframeHeight);
			parent.$("#dealInfor").parent().parent().parent().parent().parent().css("height", iframeHeight);
			parent.$("#dealInfor").parent().parent().parent().parent().parent().parent().css("height", iframeHeight);
			parent.$("#dealInfor").parent().parent().parent().parent().parent().parent().parent().css("height", iframeHeight);
		}
		catch(e) 
		{
			return;
...
<%
		boolean querymobile = false;  //是否显示用户手机
		boolean querydepuser = false;  //是否显示组/角色内人员
		String emptyAction = "追回";  //不显示人员的动作
...
%>	
  <body <%=strBodyStyle%>>
  	<div id="info_content" class="content">
		...
		%>
			...
			<TABLE cellSpacing="1" cellPadding="0" width="100%" align="center" border="0" class="tableline">
				<TBODY>
			        ...
			        <%
						//读取工单特有信息
						out.println(PreviewUtils.printMainHtmlBpp(baseFields, m_BaseModel));
					%>
				</TBODY>
			</TABLE>		
		<%
		...
		%>	
			<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="treetitle">	
				<tr style="<%=str_title_height %>;background-color:#eee;">
					<td align="left" class="treechar"></td>
					<td align="right">
					<%
	  	...
	  	<div id="div_0"></div>
				...
				while(true)
				{
					...
					String  m_str_endAction		= m_ProcessModel.getEdProcessAction();
		...
		//			long 	m_long_FlagBegin 	= m_ProcessModel.getFlagBegin();
			...
			<TABLE cellSpacing="0" cellPadding="0" id="table_<%=m_str_processid%>" width="100%" align="center" border="0" style="margin-top:5px;">
				<TBODY>
													...
													<TR <%if(!fixSub) {%> onClick="expandcoll('<%=str_Pro_infor_id %>')" onMouseOver="collonmouseover('<%=str_Pro_image_id %>')" onMouseOut="collonmouseout('<%=str_Pro_image_id %>')"<%} %>>
														<TD class="title" align="left" valign="middle" title="<%=m_str_ProDesc %>" <%if(flowID != null && !flowID.equals("")){%>style="color:red;"<%}%>>
															...
															if (m_str_ProDesc.length()>25)
															{
															 ...
															 &nbsp;
															 <span nowrap>
															 <%if(m_str_endAction.equals(emptyAction))
															 {
																 out.print("【" + m_str_endAction + "】");
															 }else{ %>
															 处理人（<%=m_str_RoleName %>）：
															<%
															...
															{
																out.print(m_str_AssginName);
																if(m_str_ActorType.equals("USER"))
																{
																	UserInfo uInfo = querymobile ? PreviewUtils.getUserInfo(m_str_AssignLoginName) : null;
																	if(uInfo != null && uInfo.getMobile() != null) {
																		out.print( "（" + uInfo.getMobile() + "）");
																	}
																}
																else
																{
																	...
																	{
																		List<UserInfo> userList = querydepuser ? depManagerService.getUserByDepID(m_ProcessModel.getAssignGroupId(), false) : null;
																	...
																	{
																		List<RoleUser> userList = querydepuser ? roleManagerService.getRoleUserByCroleID(m_ProcessModel.getAssignGroupId()) : null;
																		if(userList != null && userList.size() > 0)
																		{
																	...
																	out.print("（" + tmp_UsersInGroup + "）");
																}
															}
															else
															{
																UserInfo uInfo = querymobile ? PreviewUtils.getUserInfo(m_str_DealLoginName) : null;
																out.print(m_str_DealFullName);
																	if(uInfo != null && uInfo.getMobile() != null) {
																		out.print( "（" + uInfo.getMobile() + "）");
																	}
															}
			        ...
			        <%
						String s = PreviewUtils.printStepHtmlBpp(baseFields, m_str_processid, taskId, baseid, baseschema, tplId);
						out.println(s);
						//处理附件
