...
<html>
  <head> 
	...
	$(function() {
		...
		$("#contactperson").autocomplete($ctx+"/userManager/autoSearchExOutHtml.action", {
			extraParams: 
	        {   
	             sqlQuery: "SQL_TEST_SearchUser.query"
	        },
		...
		$("#dealGroup").autocomplete($ctx+"/depManager/autoSearchExOutHtml.action", {
			extraParams: 
	        {   
		...
		$("#cdbTeamName").autocomplete($ctx+"/depManager/autoSearchExOutHtml.action", {
			extraParams: 
	        {   
			...
			document.getElementsByName('cdbTeamId')[0].value = row[1];
		});	
		$("#cdbdealTeamName").autocomplete($ctx+"/depManager/autoSearchExOutHtml.action", {
			extraParams: 
	        {   
	             sqlQuery: "SQL_TEST_SearchTeam.query"
	        },
	...
	/*	$("#ContactDepartment").autocomplete($ctx+"/depManager/autoSearchExOutHtml.action", {
			extraParams: 
	        {   
	             sqlQuery: "SQL_TEST_SearchDept.query"
	        },
	...
	//	document.getElementsByName("createstarttime")[0].value=today+" 00:00:00";
	//	document.getElementsByName("createendtime")[0].value=today+" 23:59:59";
...
// 		  }
	  ...
	  function toSelectActor(){
		   ...
		   */
		   ...
		   //现象分类
		   function openPhenoClassTree(enableCheckBoxes,nameElementId,idElementId){
				if(!idElementId) idElementId = "";
				var url = $ctx+"/commonViewTree/commonViewTree.action?opt.query=SQL_CDB_INCIDENT_PHENOCLASS.query&opt.nameElementId="+nameElementId+"&opt.enableCheckBoxes="+enableCheckBoxes+"&opt.idElementId="+idElementId+"&opt.parentid=0&opt.sameAsId=parentid";
				deptTreeWindow = window.open(url,'deptTree','height=800px,width=600px,toolbar=no,status=no,location=no,directories=no');
			}
		   //原因分类
		   function openCauseClassTree(enableCheckBoxes,nameElementId,idElementId){
				if(!idElementId) idElementId = "";
				var url = $ctx+"/commonViewTree/commonViewTree.action?opt.query=SQL_CDB_INCIDENT_CAUSECLASS.query&opt.nameElementId="+nameElementId+"&opt.enableCheckBoxes="+enableCheckBoxes+"&opt.idElementId="+idElementId+"&opt.parentid=0&opt.sameAsId=parentid";
				deptTreeWindow = window.open(url,'deptTree','height=800px,width=600px,toolbar=no,status=no,location=no,directories=no');
			}
	</script>
  </head>
  ...
  <body style="background-color:white;">
	  	<dg:datagrid  var="waitingdealsheet"   title="${nodePath}" action="" sqlname="SQL_WF_SQProblem.query" tquery="workSheetTitle">
	  		<dg:lefttoolbar>
	  		  ...
	  		  <eoms:operate cssclass="page_saves_button" id="page_saves_button" onmouseover="this.className='page_saves_button_hover'" onmouseout="this.className='page_saves_button'"  onclick="showsearchcondition('${ctx}','SQL_WF_SQProblem.query')" text="com_btn_config" />
  			  ...
  			  <eoms:operate cssclass="page_excel_button" id="com_btn_excel" onmouseover="this.className='page_excel_button_hover'" onmouseout="this.className='page_excel_button'" onclick="excelExport('SQL_WF_SQProblem.query','${wfSortId}','${baseSchema}')" text="com_btn_export"/>
  			</dg:lefttoolbar>
	  	<dg:condition>
		  	<div align="center">
		        ...
		        <tr>
		          <td colspan="6" align="center">
		        	...
		        	<input type="button" name="resetCondition" id="resetCondition" value="<eoms:lable name="com_btn_reset"/>" class="ResetButton" onmouseover="this.className='ResetButton_hover'" onmouseout="this.className='ResetButton'" onclick="z_reset()"/>
		          	<input type="button" name=saveCondition id="saveCondition"  class="searchButton" onmouseover="this.className='searchButton_hover'" onmouseout="this.className='searchButton'" onclick="saveConditions('problem')" value="收藏"/>
		        	<input type="button" name=defaultCondition id="defaultCondition"  class="searchButton" onmouseover="this.className='searchButton_hover'" onmouseout="this.className='searchButton'" onclick="setConditions('problem','${userSession.pid}')" value="提取 "/>
		          </td>
		        </tr>
		      </table>
			</div>
