...
<html>
		 ...
		 */
		  ...
		  */
		   ...
		   */
		   ...
		   //应用系统
		   function openConnBusSystemTree(enableCheckBoxes,nameElementId,idElementId){
				if(!idElementId) idElementId = "";
				var url = $ctx+"/commonViewTree/commonViewTree.action?opt.query=SQL_CDB_SYSTEM.query&opt.nameElementId="+nameElementId+"&opt.enableCheckBoxes="+enableCheckBoxes+"&opt.idElementId="+idElementId+"&opt.parentid=0&opt.sameAsId=parentid";
				deptTreeWindow = window.open(url,'deptTree','height=800px,width=600px,toolbar=no,status=no,location=no,directories=no');
			}
		   //发布类别
		   function openRelease_typeTree(enableCheckBoxes,nameElementId,idElementId){
				if(!idElementId) idElementId = "";
				var url = $ctx+"/commonViewTree/commonViewTree.action?opt.query=SQL_CDB_Release_type.query&opt.nameElementId="+nameElementId+"&opt.enableCheckBoxes="+enableCheckBoxes+"&opt.idElementId="+idElementId+"&opt.parentid=0&opt.sameAsId=parentid";
				deptTreeWindow = window.open(url,'deptTree','height=800px,width=600px,toolbar=no,status=no,location=no,directories=no');
			}
	</script>
  </head>
  ...
  <body style="background-color:white;">
	  	<dg:datagrid  var="waitingdealsheet"   title="${nodePath}" action="" sqlname="SQL_WF_SQRelease.query" tquery="workSheetTitle">
	  		<dg:lefttoolbar>
	  		  ...
	  		  <eoms:operate cssclass="page_saves_button" id="page_saves_button" onmouseover="this.className='page_saves_button_hover'" onmouseout="this.className='page_saves_button'"  onclick="showsearchcondition('${ctx}','SQL_WF_SQRelease.query')" text="com_btn_config" />
  			  ...
  			  <eoms:operate cssclass="page_excel_button" id="com_btn_excel" onmouseover="this.className='page_excel_button_hover'" onmouseout="this.className='page_excel_button'" onclick="excelExport('SQL_WF_SQRelease.query','${wfSortId}','${baseSchema}')" text="com_btn_export"/>
  			</dg:lefttoolbar>
	  	<dg:condition>
		  	<div align="center">
		      <table  class="serachdivTable" style="width:80%">
		        <tr>
		          <td colspan="6" align="center">
		        	...
		        	<input type="button" name=saveCondition id="saveCondition"  class="searchButton" onmouseover="this.className='searchButton_hover'" onmouseout="this.className='searchButton'" onclick="saveConditions('release')" value="收藏 "/>
		        	<input type="button" name=defaultCondition id="defaultCondition"  class="searchButton" onmouseover="this.className='searchButton_hover'" onmouseout="this.className='searchButton'" onclick="setConditions('release','${userSession.pid}')" value="提取 "/>
		          </td>
		        </tr>
		      </table>
			</div>
	    ...
	    <dg:gridtitle>
		    	<tr>
		    		<th width="11%"><eoms:lable name="wf_sheet_serialnum"/></th>
		    		<th width="18%">主题</th>
		    		<th width="10%"><eoms:lable name="wf_sheet_creator"/></th>
		    		<th width="10%"><eoms:lable name="wf_sheet_deal"/></th>
		    		<th width="12%"><eoms:lable name="wf_sheet_createtime"/></th>
		    		<th width="12%"><eoms:lable name="wf_sheet_closetime"/></th>
