		...
		<dg:datagrid  var="eventHandlerComp" sqlname="SQL_WF_EventHandlerCompList.query" title="${nodePath}">
	  		<dg:lefttoolbar>
				...
				<eoms:operate cssclass="page_add_button" id="com_btn_add"
					...
					onclick="openwindow('${ctx}/smsFilter/addSmsFilter.action','',800,500);"
			...
			<dg:gridrow>
				...
				<tr class="${agency_row}" style="cursor: hand">
					...
					<td onclick="edit('${pid}');">${businesstype}</td>
					<td onclick="edit('${pid}');">${bussinessmark}</td>		
					<td onclick="edit('${pid}');">${usertype}</td>	
					<td onclick="edit('${pid}');">${holidaystatus}</td>							
				</tr>
			</dg:gridrow>
		</dg:datagrid>			
	</body>
