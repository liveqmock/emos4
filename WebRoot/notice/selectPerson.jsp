	...
	<head>
		...
		*/
		function getItemInfo()
		{
			...
			window.opener.document.getElementById("selectUser").value=str_temp;
			
			var showStr = '';
			if(str_temp!=''){
				var tempArr = str_temp.split(';');
				for (var i = 0; i < tempArr.length; i++) {
				    if(tempArr[i].indexOf(",") > 0 ) {
				    	var selTemp = tempArr[i].split(',');
				    	showStr = showStr +  selTemp[1] + ';';
				    }
				}
			}
			
			window.opener.document.getElementById("noticeInfo.customUser").value=showStr;
			window.close();
		}
		</script>
	</head>
	...
	<body>
		...
		<div class="add_bottom">
			<input type="button" value="确定" class="save_button"
				...
				onmouseout="this.className='save_button'" onclick="getItemInfo();"
