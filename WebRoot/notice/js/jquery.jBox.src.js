...
(function ($) {
    $.jBox = function (content, options) {
        ...
        boxHtml.push('<div id="jbox" class="jbox" style="position:absolute;width:auto;height:auto;opacity:0.8;filter:alpha(opacity=80);">');
    ...
    $.jBox.messagerDefaults = { content: '', title: 'jBox', icon: 'none', width: 350, height: 100, timeout: 4000, showType: 'slide', showSpeed: 600, border: 0, buttons: {}, buttonsFocus: 0, loaded: function () { }, submit: function (v, h, f) { return true; }, closed: function () { } };
    ...
    $.jBox.close = function (token, boxType,height) {
        token = token || false;
        ...
        if (typeof token == 'string') {
            ...
            if (token) {
                ...
                if (boxs.length > 0) {
					...
					var i = 0;
					jboxe.each(function(){
						jboxe.eq(i).css("top",document.documentElement.clientHeight-height*(boxs.length-i+1)+"px");
						i++;
					});
                }
            }
    ...
    $.jBox.closeTip = function () {
        $.jBox.close(false, 'tip', 0);
    };

    $.jBox.messager = function (content, title, timeout, options) {
        $.jBox.closeMessager(options.height);
        var defaults = {
            content: content,
            title: title,
    ...
    $.jBox.closeMessager = function (height) {
        $.jBox.close(false, 'messager',height);
    };
