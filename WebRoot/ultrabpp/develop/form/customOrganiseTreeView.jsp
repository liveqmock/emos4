  ...
  <body>
  	<div class="content">
		...
		<script type="text/javascript">
			...
			if(sendTreeId!="")
			{
				tree.setXMLAutoLoading("${ctx}/ultrabpp/assigntree/assignTreeOrgnized.action?isSelectType=<%=isSelectType%>&formCustSendTree=<%=sendTreeId%>");   
				tree.loadXML("${ctx}/ultrabpp/assigntree/assignTreeOrgnized.action?id=0&rearchUserOrDep=<%=URLEncoder.encode(URLEncoder.encode(rearchUserOrDep,"UTF-8"),"utf-8")%>&isSelectType=<%=isSelectType%>&formCustSendTree=<%=sendTreeId%>");
			}
			...
			function getDepAndUser()//返回选择的部门和人
			{
				...
				if(ids!=''){
					...
					for (var i = 0; i < idArr.length; i++) {
						...
						//fanying 2013-4-24 解决自定义派发树同一个人员出现两次的问题 增加父节点id
						returnStr += orgtype+':'+pid+':'+parentid+';';
					}
				}
			}
