...
public class ProcessUtil{
	 ...
	 public static String getStringFromMapIgnoreCase(String key, Map inputs) {
		 String rtn = "";
		 if (inputs != null) {
			for (Iterator iter = inputs.keySet().iterator(); iter.hasNext();) {
				String k = (String) iter.next();
				if (StringUtils.isNotBlank(k) && k.equalsIgnoreCase(key)) {
					Object obj = inputs.get(k);
					if (obj != null) {
						rtn = obj.toString();
						break;
					}
	...
	public static Object reflect(Object obj, String mthName, Object[] params) {
		Object rtn = null;
		if (obj != null) {
			...
			if (methods != null) {
				for (int i = 0; i < methods.length; i++) {
					...
					if (mthName.equals(name)) {
						try {
							rtn = methods[i].invoke(obj, params);
