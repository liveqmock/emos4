...
@Transactional
public class HibernateMapDaoImpl<T> implements IDao<T> {
	...
	public HibernateMapDaoImpl() {
		entityClass = (Class) java.util.Map.class;
	}
	...
	public Serializable getId(final T entity) {
		...
		try {
			idValue = (Serializable) PropertyUtils.getProperty(entityName,
					getIdName());
	...
	public Page pagedQuery(final Query query, final int pageNo,
			final int pageSize) {
