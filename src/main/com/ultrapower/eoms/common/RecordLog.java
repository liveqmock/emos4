package com.ultrapower.eoms.common;
...
public class RecordLog{
	 ...
	 * @param url 受访Url
	 */
	public static void printFluxStatLog(String url, String nodeName){
		DataRow p_dtrow = new DataRow();
		p_dtrow.put("PID", UUIDGenerator.getUUIDoffSpace());
		UserSession userSession = ActionContext.getUserSession();
		if(userSession!=null){
			p_dtrow.put("LOGINNAME", StringUtils.checkNullString(userSession.getLoginName()));
			p_dtrow.put("IP", StringUtils.checkNullString(ActionContext.getRequest().getRemoteAddr()));
			p_dtrow.put("USERNAME", StringUtils.checkNullString(userSession.getFullName()));
			p_dtrow.put("DEPID", StringUtils.checkNullString(userSession.getDepId()));
			p_dtrow.put("DEPNAME", StringUtils.checkNullString(userSession.getDepName()));
			p_dtrow.put("COMPANYNAME", StringUtils.checkNullString(userSession.getCompanyName()));
			p_dtrow.put("NODENAME", nodeName);
		}else{
			p_dtrow.put("LOGINNAME", "");
			p_dtrow.put("IP", "");
			p_dtrow.put("USERNAME", "");
			p_dtrow.put("DEPID", "");
			p_dtrow.put("DEPNAME", "");
			p_dtrow.put("COMPANYNAME", "");
			p_dtrow.put("NODENAME", "");
		}
		p_dtrow.put("URL", url);
		p_dtrow.put("TIME", System.currentTimeMillis()/1000);
	...
	public static void printLog(String description,int level,Throwable e)
	{

		switch(level)
		{
			case LOG_LEVEL_ERROR:
				loger.error(description,e);
				break;
			case LOG_LEVEL_WARN:
				loger.warn(description,e);
				break;
			case LOG_LEVEL_INFO:
				loger.info(description,e);
				break;
			case LOG_LEVEL_DEBUG:
				loger.debug(description,e);
				break;
			case LOG_LEVEL_TRACE:
				loger.trace(description,e);
				break;
			default:
				loger.info(description,e);
		}
