package com.ultrapower.wfinterface;
...
public class RecordLog{
	...
	public static void printOperateAuditLog(String module,String operate,String description) 
	{
		...
		if(resultrowsum>0){
			loger.debug("记录操作审计信息成功!");
		}else{
			loger.debug("记录操作审计信息失败!");
		}
	}
	
	/**
	 * 告警日志入库
	 * @param bsTCmbcAlarmnote
	 */
	public static void pringAlarmLog(BsTCmbcAlarmnote bsTCmbcAlarmnote){
		DataRow p_dtrow = new DataRow();
		p_dtrow.put("PID", UUIDGenerator.getUUIDoffSpace());
		p_dtrow.put("ALARMID", bsTCmbcAlarmnote.getAlarmid());
		p_dtrow.put("ALARMTITLE", bsTCmbcAlarmnote.getAlarmtitle());
		p_dtrow.put("ALARMDESC", bsTCmbcAlarmnote.getAlarmdesc());
		p_dtrow.put("ALARMSYSTYPE", bsTCmbcAlarmnote.getAlarmsystype());
		p_dtrow.put("ALARMONETYPE", bsTCmbcAlarmnote.getAlarmonetype());
		p_dtrow.put("ALARMTWOTYPE", bsTCmbcAlarmnote.getAlarmtwotype());
		p_dtrow.put("ALARMTHREETYPE", bsTCmbcAlarmnote.getAlarmthreetype());
		p_dtrow.put("ALARMLOCAL", bsTCmbcAlarmnote.getAlarmlocal());
		p_dtrow.put("ALARMHAPPENTIME", bsTCmbcAlarmnote.getAlarmhappentime());
		p_dtrow.put("ALARMSTARTTIME", bsTCmbcAlarmnote.getAlarmstarttime());
		p_dtrow.put("DEP", bsTCmbcAlarmnote.getDep());
		p_dtrow.put("ISCREATESHEET", bsTCmbcAlarmnote.getIscreatesheet());
		p_dtrow.put("FAULTLOG", bsTCmbcAlarmnote.getFaultlog());
		p_dtrow.put("INPUTTIME", System.currentTimeMillis()/1000);
		int resultrowsum = dataAdapter.putDataRow("BS_T_CMBC_ALARMNOTE", p_dtrow, null, null);
		if(resultrowsum>0){
			loger.debug("记录告警信息成功!");
		}else{
			loger.debug("记录告警信息失败!");
		}
	}
	 ...
	 * @param bsTCmbcAlarmnote
	 */
	public static void pringAlarmLogUpdate(BsTCmbcAlarmnote bsTCmbcAlarmnote){
		DataRow p_dtrow = new DataRow();
		p_dtrow.put("PID", bsTCmbcAlarmnote.getPid());
		p_dtrow.put("ALARMID", bsTCmbcAlarmnote.getAlarmid());
		p_dtrow.put("ALARMTITLE", bsTCmbcAlarmnote.getAlarmtitle());
		p_dtrow.put("ALARMDESC", bsTCmbcAlarmnote.getAlarmdesc());
		p_dtrow.put("ALARMSYSTYPE", bsTCmbcAlarmnote.getAlarmsystype());
		...
		int resultrowsum = dataAdapter.putDataRow("BS_T_CMBC_ALARMNOTE", p_dtrow, null, null);
		if(resultrowsum>0){
			loger.debug("修改告警信息成功!");
		}else{
			loger.debug("修改告警信息失败!");
		}
	}
	...
	public static void printFluxStatLog(String url){
		...
		if(resultrowsum>0){
			loger.debug("记录流量统计日志成功!");
		}else{
			loger.debug("记录流量统计日志失败!");
		}
	}
